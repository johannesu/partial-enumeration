...
class TypePatchConsSparseSorted
{
private:
...
public:
	...
	static unsigned int twopow(int K)
	{
...
private:
friend class MRFEnergy<TypePatchConsSparseSorted>;
...
inline TypePatchConsSparseSorted::LocalSize::LocalSize(int K)
{
}

inline TypePatchConsSparseSorted::GlobalSize::GlobalSize(	int num_labels, 
															int patchSize,
															int * labels,
															int * hor_source,
															int * hor_sink,
...
inline void TypePatchConsSparseSorted::SortHorizontal(int* labels, 
																								 			int* sortedLabels, 
																								 			int num_labels, 
																								 			int patchSize,
																								 			bool issource)
	...
	if (issource)
	{
		for (i = 0; i < twopow(patchSize*patchSize) ; i++)
		{
			if ((mask & i) == i) // Search for all patches consistent with this patch.
			{
				...
				{
					if ((((unsigned int)labels[j]) & mask) == i)
					{
						*lastPos = j;
						++lastPos;
					}
...
inline void TypePatchConsSparseSorted::SortVertical(int* labels, 
																						  int* sortedLabels, 
																						 	int num_labels, 
																						 	int patchSize,
																						 	bool issource)
...
inline TypePatchConsSparseSorted::NodeData::NodeData(REAL* data)
{
	m_data = data;
}


inline TypePatchConsSparseSorted::EdgeData::EdgeData(Type type)
{
...
inline int TypePatchConsSparseSorted::Vector::GetSizeInBytes(GlobalSize Kglobal, LocalSize K)
{
	if (Kglobal.m_K < 1)
	{
		return -1;
...
inline void TypePatchConsSparseSorted::Vector::Initialize(GlobalSize Kglobal, LocalSize K, NodeData data)
{
	memcpy(m_data, data.m_data, Kglobal.m_K*sizeof(REAL));
}

inline void TypePatchConsSparseSorted::Vector::Add(GlobalSize Kglobal, LocalSize K, NodeData data)
{
...
inline void TypePatchConsSparseSorted::Vector::SetZero(GlobalSize Kglobal, LocalSize K)
{
	memset(m_data, 0, Kglobal.m_K*sizeof(REAL));
}

inline void TypePatchConsSparseSorted::Vector::Copy(GlobalSize Kglobal, LocalSize K, Vector* V)
{
	memcpy(m_data, V->m_data, Kglobal.m_K*sizeof(REAL));
}

inline void TypePatchConsSparseSorted::Vector::Add(GlobalSize Kglobal, LocalSize K, Vector* V)
{
	for (int k=0; k<Kglobal.m_K; k++)
	{
		m_data[k] += V->m_data[k];
...
inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Vector::GetValue(GlobalSize Kglobal, LocalSize K, Label k)
{
	assert(k>=0 && k<Kglobal.m_K);
	return m_data[k];
}

inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Vector::ComputeMin(GlobalSize Kglobal, LocalSize K, Label& kMin)
{
	REAL vMin = m_data[0];
	kMin = 0;
...
inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Vector::ComputeAndSubtractMin(GlobalSize Kglobal, LocalSize K)
{
	REAL vMin = m_data[0];
	for (int k=1; k<Kglobal.m_K; k++)
	{
...
inline int TypePatchConsSparseSorted::Vector::GetArraySize(GlobalSize Kglobal, LocalSize K)
{
	return Kglobal.m_K;
}

inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Vector::GetArrayValue(GlobalSize Kglobal, LocalSize K, int k)
{
	assert(k>=0 && k<Kglobal.m_K);
	return m_data[k];
}

inline void TypePatchConsSparseSorted::Vector::SetArrayValue(GlobalSize Kglobal, LocalSize K, int k, REAL x)
{
	assert(k>=0 && k<Kglobal.m_K);
	m_data[k] = x;
}
...
inline int TypePatchConsSparseSorted::Edge::GetSizeInBytes(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj, EdgeData data)
{
...
inline int TypePatchConsSparseSorted::Edge::GetBufSizeInBytes(int vectorMaxSizeInBytes)
{
	return vectorMaxSizeInBytes;
}

inline void TypePatchConsSparseSorted::Edge::Initialize(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj, EdgeData data, Vector* Di, Vector* Dj)
{
	m_type = data.m_type;
	int* dataptr;
...
inline TypePatchConsSparseSorted::Vector* TypePatchConsSparseSorted::Edge::GetMessagePtr()
{
	return m_message;
}

inline void TypePatchConsSparseSorted::Edge::Swap(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj)
{
...
inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Edge::UpdateMessage(GlobalSize Kglobal, LocalSize Ksource, LocalSize Kdest, Vector* source, REAL gamma, int dir, void* _buf)
{
	...
	{
		int ksource, kdest;
		unsigned int mask, i, destLabel, sourceLabel, destInd, sourceInd;
		...
		if (dir == ((EdgePatchConsSparseSorted*)this)->m_dir)
		{
			...
			while (i<Kglobal.m_K)
			{
				...
				m_message->m_data[destInd] = std::numeric_limits<TypePatchConsSparseSorted::REAL>::infinity();
...
inline void TypePatchConsSparseSorted::Edge::AddColumn(GlobalSize Kglobal, LocalSize Ksource, LocalSize Kdest, Label ksource, Vector* dest, int dir)
{
...
inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Edge::horintcost(int i, int j, int dir, int K,int patchSize)
{
	unsigned int k,mask;
	...
	if (dir == 0)
	{
		if (((i & mask) >> 1) == (j & (mask >> 1)))
			...
			return std::numeric_limits<TypePatchConsSparseSorted::REAL>::infinity();
...
inline TypePatchConsSparseSorted::REAL TypePatchConsSparseSorted::Edge::vertintcost(int i, int j, int dir, int K, int patchSize)
{
	unsigned int k,mask;
